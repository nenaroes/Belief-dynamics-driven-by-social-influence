...
globals [
  ...
  ;; variables for making a heatmap in Python
  zero
  one
  two
  three
...
to setup-globals
  ...
  set i 0
  set #Cars ( PercCar / 100 * NumStudents )
  if #Cars < 1 [set #Cars 1]
end
...
to SetFileName
  ;; Per travel type, a different file is consulted. The required file for a specific type is consulted here
  file-close
  ask students [
  let ws "ModelData"
  let xs name
...
to data
  ;; Opening the files that contain agents' beliefs on value-mode relations.
  ;; Allocating initial beliefs to every mode - value combination
  file-open (FileName)
...
to combine
 ...
 foreach personalval-list [
    [val] ->
    ;; The values found in the file are transferred to agent-specific beliefs.
...
;;Setup competences and affordances of the different actions
;;CAR
...
ask activities with [name = "Take bus to lecture"] [
    ...
    create-has-affordances-with affordances with [name = "bus" or name = "road" or name = "rain gear"] ]
;;BIKE
...
;;SETUP STUDENT COMPETENCES
  ...
  if ( ((PercLicense / 100) - licenses) * count (students with [car? != true] ) > 0 ) [ask n-of (((PercLicense / 100) - licenses) * count (students with [car? != true] )) students [set license? true] ]
...
to InventoryCarpoolers
  ...
  ask students [ if ticks > 0 [
    ...
    ;; with a value for traveltogether of 3, an agent will always want to travel together
    ifelse traveltogether = 3 [set partnerup true ] [
     ;; With a traveltogether of 2, there is a 50% change of agents wanting to travel with others
      ifelse traveltogether = 2 [ifelse  (random 100) > 50 [set partnerup true ] [set partnerup false]]
      ;; With a lower traveltogether value, an agent does not want to travel with others
      [set partnerup false]]
    ;; Here, agents make an inventory if there are agents with a car who are willing to take others with them
   ask drivers [if partnerup = true and carpool? = true and member? "car" belongings  [set driveoption 1]] ] ]
end

to SortValues
  ask students [
    ;; The value-priority system is set here. The importance given to a value determines the amount of times it is prioritised to choose from the perspective of satisfying this value.
...
to update-totalval
  ;; Once all "valuepriority-points" are used in their destined way, the total valuepriority-points is restored
  ask my-Stud-strengths [set new-valuestr value-strength]
...
to MatchAct
  ;;firstly, the students that have a car decide whether or not they will choose the car as their transport mode, so others can anticipate on this fact
  foreach sort-on [( - driveoption)] students [
    ...
    foreach [personalval-list] of stud [
      ...
      if item 0 x = [name] of target-value
        [ ask target-value
          ;; give the mode-value relation the number of points that the agent gives to this relation concerning the agent's beliefs
          [ask my-act-strengths [
           ifelse [name] of other-end = "Bike to lecture" [
            ...
            ;; now these beliefs are ranked. Since the mode that can satisfy the target value the best at the agent's beliefs, this one has it's preference
            foreach sort-on [(- preference)] my-act-strengths [
              [str] ->
              ask myself [
                ;; determine how much room there is for others to carpool when they do not have a car themselves
               let otherdrivers count (other students with [driveoption = 1 and [name] of implement = "Drive to lecture"  and partnerup = true ] )
                ...
                ;; to get the option to carpool, one needs to have a peer that offers their car, but the decision-maker also needs to be willing to travel together.
                if  (member? "Drive to lecture" not-option  and carpool? = true and otherdrivers > 0 ) [
                  ;; this variable helps detecting the right place where "drive to lecture" is located in the not-option list, so it can be removed (since driving does become an option through carpooling)
                set p 0
                    foreach not-option [
              ...
              ;;For each preference-sorted transport mode, check if it is an option. If not, continue with next preference. If so; the 'chosen' variable will be set true, leading to the implement variable not to be changed any more.
...
to Decide
  ...
  set color [color] of implement
    set ChosenMode [name] of implement
...
ask students [
    ...
    ;; determining the enjoyment and stress in the previous rounds and the avarage of these values
    set TotalEnjoyment lput Enjoyment TotalEnjoyment
    set TotalStress lput Stress TotalStress

    ;; Agents can remember their satisfaction from previous choices for 20 ticks
    if history > 20 [set totalenjoyment but-first totalenjoyment
      set totalstress but-first totalstress]

     let PreviousEnjoy mean TotalEnjoyment
     let PreviousStress mean TotalStress
...
ask students [
    ...
    ask other students [
      let difference (map - [specificvalues] of myself specificvalues)
...
;;;      set focusagent self
  ...
  ;; Compare your own stress and enjoyment to that of other agents you find important (i.e., are part of an agents 'like-minded' group)
    ...
    foreach [StressEnjoyDifference] of groupmember [
      level ->  set averagesocialdiff lput (( StressEnjoyDifference - level ) / numgroupmembers )  averagesocialdiff
    ]
    ...
    if like-minded != 0 [
     foreach like-minded [
        ...
        ask targetagent [
          ...
          ask myself [
              set BeliefDifference (map - bikebeliefs [bikebeliefs] of myself)
          set correspondingbeliefs bikebeliefs] ]

      if [name] of implement = "Drive to lecture" [
          ask myself [
              set BeliefDifference (map - carbeliefs [carbeliefs] of myself)
          set correspondingbeliefs carbeliefs] ]

      if [name] of implement = "Take bus to lecture" [
          ask myself [
              set BeliefDifference (map - busbeliefs [busbeliefs] of myself)
...
ask myself [
     ...
     ;;     Determine direction of belief-update: ask if the difference is positive (and the other agent thus has a higher value for the belief in this relation)
              if subdiff > 0.05 [
                set  correspondingbeliefs replace-item p correspondingbeliefs ((item p correspondingbeliefs) - beliefshift)
  ...
  ask students [
    foreach personalval-list [
      ...
      if inspect_variable = item 0 pers [
        ;; Register the strength given to the inspect_variable value
        ask my-stud-strengths with [[name] of other-end = item 0 pers] [ask myself [set Vstrength [value-strength] of myself] ]
...
to CountSimilar
  ...
  ask students [
  set meanbelief ( mean bikebeliefs + mean busbeliefs + mean carbeliefs ) / 3]



  if everyonehascar = true [
    ask students [
      if not member? "car" belongings [
      set belongings lput "car" belongings] ] ]


;; These variables serve to create a heat plot with the amount of students that have a specific belief in this point in the simulation
...
"" ""
PENS
"Bike" 1.0 0 -13840069 true "" "plot (count students with [color = green]) / NumStudents"
"Car" 1.0 0 -13791810 true "" "plot count students with [color = blue] / NumStudents"
"Bus" 1.0 0 -1184463 true "" "plot count students with [color = yellow] / NumStudents "
...
"" ""
PENS
"Enjoyment" 1.0 0 -1264960 true "" "plot [Enjoyment] of student 8"
"(-) Stress" 1.0 0 -526419 true "" "plot -1 * [Stress] of student 8"
"Intrapersersonal Comp" 1.0 0 -4528153 true "" "plot [IntraPersSat] of student 8"
"Social Comp" 1.0 0 -408670 true "" "plot [SocialComp] of student 8"
"Travel Satisfaction" 1.0 0 -13840069 true "" "plot [satisfaction] of student 8"
...
"" ""
PENS
"default1" 1.0 0 -16777216 true "" "plot AllStudents_TotalTickChangeBel"
...
1765
269
Mean beliefs over all value-transport combinations
NIL
NIL
0.0
10.0
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot [meanbelief] of student 7"
"pen-1" 1.0 0 -7500403 true "" "plot [meanbelief] of student 8"
"pen-2" 1.0 0 -2674135 true "" "plot [meanbelief] of student 9"
"pen-3" 1.0 0 -955883 true "" "plot [meanbelief] of student 10"
"pen-4" 1.0 0 -6459832 true "" "plot [meanbelief] of student 11"
"pen-5" 1.0 0 -1184463 true "" "plot [meanbelief] of student 12"
"pen-6" 1.0 0 -10899396 true "" "plot [meanbelief] of student 13"
"pen-7" 1.0 0 -13840069 true "" "plot [meanbelief] of student 14"
"pen-8" 1.0 0 -14835848 true "" "plot [meanbelief] of student 15"
"pen-9" 1.0 0 -11221820 true "" "plot [meanbelief] of student 16"
"pen-10" 1.0 0 -13791810 true "" "plot [meanbelief] of student 17"
"pen-11" 1.0 0 -13345367 true "" "plot [meanbelief] of student 18"
...
## RELATED MODELS
This model has an extended version with agents that have a theory of mind capacity. That means, that agents vary in their skills of correct conjectures on others' private characteristics. 
...
@#$#@#$#@
<experiments>
  <experiment name="REFERENCE" repetitions="1" sequentialRunOrder="false" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="1501"/>
    <metric>[name] of student 7</metric>
    <metric>[name] of student 8</metric>
    <metric>[name] of student 9</metric>
    <metric>[name] of student 10</metric>
    <metric>[name] of student 11</metric>
    <metric>[name] of student 12</metric>
    <metric>[name] of student 13</metric>
    <metric>[name] of student 14</metric>
    <metric>[name] of student 15</metric>
    <metric>[name] of student 16</metric>
    <metric>[name] of student 17</metric>
    <metric>[name] of student 18</metric>
    <metric>[name] of student 19</metric>
    <metric>[name] of student 20</metric>
    <metric>[name] of student 21</metric>
    <metric>[name] of student 22</metric>
    <metric>[name] of student 23</metric>
    <metric>[name] of student 24</metric>
    <metric>[name] of student 25</metric>
    <metric>[name] of student 26</metric>
    <metric>[plot_belief] of student 7</metric>
    <metric>[plot_belief] of student 8</metric>
    <metric>[plot_belief] of student 9</metric>
    <metric>[plot_belief] of student 10</metric>
    <metric>[plot_belief] of student 11</metric>
    <metric>[plot_belief] of student 12</metric>
    <metric>[plot_belief] of student 13</metric>
    <metric>[plot_belief] of student 14</metric>
    <metric>[plot_belief] of student 15</metric>
    <metric>[plot_belief] of student 16</metric>
    <metric>[plot_belief] of student 17</metric>
    <metric>[plot_belief] of student 18</metric>
    <metric>[plot_belief] of student 19</metric>
    <metric>[plot_belief] of student 20</metric>
    <metric>[plot_belief] of student 21</metric>
    <metric>[plot_belief] of student 22</metric>
    <metric>[plot_belief] of student 23</metric>
    <metric>[plot_belief] of student 24</metric>
    <metric>[plot_belief] of student 25</metric>
    <metric>[plot_belief] of student 26</metric>
  ...
  <experiment name="GroupSize" repetitions="100" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="1501"/>
    <metric>[name] of student 7</metric>
  ...
  <experiment name="SimilarityBoundary" repetitions="10" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="1501"/>
    <metric>[name] of student 7</metric>
    <metric>[plot_belief] of student 7</metric>
    <metric>[initialbeliefs] of student 7</metric>
    <metric>[finalbeliefs] of student 7</metric>
    <metric>AllStudents_TotalTickChangeBel</metric>
  ...
  <experiment name="CountStud" repetitions="10" sequentialRunOrder="false" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="1500"/>
    <metric>[plot_belief] of student 7</metric>
    <metric>[CountSimStud] of student 7</metric>
    <metric>[ChosenMode] of student 7</metric>
    <enumeratedValueSet variable="PercBike">
      <value value="75"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="NumStudents">
  ...
  <experiment name="experiment2" repetitions="100" runMetricsEveryStep="true">
    ...
    <enumeratedValueSet variable="everyonehascar">
      <value value="true"/>
      <value value="false"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="show-names">
  ...
  <experiment name="experiment" repetitions="100" sequentialRunOrder="false" runMetricsEveryStep="true">
    ...
    <enumeratedValueSet variable="PercLicense">
      <value value="60"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="hide-competences">
      <value value="true"/>
  ...
  <experiment name="REFERENCE_100" repetitions="100" sequentialRunOrder="false" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="1501"/>
    <metric>[plot_belief] of student 7</metric>
  ...
  <experiment name="ValueStrength" repetitions="100" sequentialRunOrder="false" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="1501"/>
    <metric>[plot_belief] of student 7</metric>
    <metric>[name] of student 7</metric>
    <metric>[Vstrength] of student 7</metric>
    <enumeratedValueSet variable="PercBike">
      <value value="75"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="NumStudents">
  ...
  <experiment name="ST100" repetitions="100" sequentialRunOrder="false" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="1501"/>
    <metric>[name] of student 7</metric>
